...
describe('desktop-core client bundle', () => {
  it('registers a profiles settings section through the public client module contract', async () => {
    ...
    expect(definition?.id).toBe('@dsh-workbench/desktop-core')
    const client = definition?.factory((name) => {
      if (name === 'react') return { createElement: vi.fn() }
