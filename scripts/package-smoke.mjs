...
function validateReport(report, context) {
  ...
  assertSmoke(Number.isSafeInteger(report.runtime?.platformSchemaVersion), 'Packaged platform schema was not verified')
  assertSmoke(typeof report.runtime?.platformDatabase === 'string', 'Packaged platform database path was not verified')
  assertSmoke(report.runtime?.platformPersistenceVerified === true, 'Packaged platform persistence was not verified')
  assertSmoke(report.runtime?.taskPlatformUiMounted === true, 'Packaged task platform UI was not mounted')
  ...
  const taskPlatformEntry = resolve(report.runtime.taskPlatformEntry)
  const platformDatabase = resolve(report.runtime.platformDatabase)
  ...
  assertSmoke(isPathInside(packagedAppPath, taskPlatformEntry), 'Smoke task platform escaped copied resources')
  assertSmoke(isPathInside(userDataPath, platformDatabase), 'Smoke platform database escaped isolated user data')
  ...
  assertSmoke(!isPathInside(root, taskPlatformEntry), 'Packaged task platform resolved inside the source checkout')
}
