...
describe('prepareProfileModuleFallback', () => {
  it('creates and heals the namespaced bare-package link used by DSH profiles', async () => {
    ...
    const taskPlatformLink = join(dshHome, 'profiles', 'node_modules', '@dsh-workbench', 'task-platform')
    const expectedTaskPlatform = dirname(require.resolve('@dsh-workbench/task-platform/package.json'))
    expect((await lstat(taskPlatformLink)).isSymbolicLink()).toBe(true)
    expect(await realpath(taskPlatformLink)).toBe(await realpath(expectedTaskPlatform))
